...
int enable_deny() {
    ...
    SuperSUD::Get().set_db_setting(DbEntryKey::DenylistConfig, true);
    return DenyResponse::OK;
}
...
int disable_deny() {
    ...
    SuperSUD::Get().set_db_setting(DbEntryKey::DenylistConfig, false);
    return DenyResponse::OK;
}

void initialize_denylist() {
    if (!denylist_enforced) {
        if (SuperSUD::Get().get_db_setting(DbEntryKey::DenylistConfig))
            enable_deny();
    }
}
