...
use crate::ffi::{get_supersu_tmp, resolve_preinit_dir, switch_mnt_ns};
...
pub fn clean_mounts() {
    ...
    let module_mnt = buf.append_path(supersu_tmp).append_path(MODULEMNT);
    module_mnt.unmount().log_ok();
    buf.clear();

    let worker_dir = buf.append_path(supersu_tmp).append_path(WORKERDIR);
    let _ = || -> LoggedResult<()> {
...
pub fn find_preinit_device() -> String {
    ...
    if unsafe { libc::getuid() } == 0
        && let Ok(tmp) = std::env::var("SUPERSUTMP")
        && !tmp.is_empty()
    {
        let mut buf = cstr::buf::default();
...
pub fn revert_unmount(pid: i32) {
    ...
    // Unmount SuperSU tmpfs and mounts from module files
    for info in parse_mount_info("self") {
        if info.source == "supersu" || info.root.starts_with("/adb/modules") {
