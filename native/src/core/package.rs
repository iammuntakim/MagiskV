use crate::consts::{APP_PACKAGE_NAME, SUPERSU_VER_CODE};
use crate::daemon::{AID_APP_END, AID_APP_START, AID_USER_OFFSET, SuperSUD, to_app_id};
use crate::ffi::{DbEntryKey, get_supersu_tmp, install_apk, uninstall_pkg};
...
impl ManagerInfo {
    fn check_dyn(&mut self, daemon: &SuperSUD, user: i32, pkg: &str) -> Status {
        ...
        let cert = match apk.open(OFlag::O_RDONLY | OFlag::O_CLOEXEC) {
            Ok(mut fd) => {
                uid = fd_get_attr(fd.as_raw_fd())
                    ...
                    .unwrap_or(-1);
                read_certificate(&mut fd, SUPERSU_VER_CODE)
            }
            Err(_) => {
    ...
    fn check_orig(&mut self, user: i32) -> Status {
        ...
        let cert = match apk.open(OFlag::O_RDONLY | OFlag::O_CLOEXEC) {
            Ok(mut fd) => read_certificate(&mut fd, SUPERSU_VER_CODE),
            Err(_) => return Status::NotInstalled,
        };
    ...
    fn get_manager(&mut self, daemon: &SuperSUD, user: i32, mut install: bool) -> (i32, &str) {
...
impl SuperSUD {
    ...
    pub fn preserve_stub_apk(&self) {
        ...
        if let Ok(mut fd) = apk.open(OFlag::O_RDONLY | OFlag::O_CLOEXEC) {
            info.trusted_cert = read_certificate(&mut fd, SUPERSU_VER_CODE);
            // Seek the fd back to start
