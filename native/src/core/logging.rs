...
fn android_log_write(level: LogLevel, msg: &Utf8CStr) {
    unsafe {
        __android_log_write(level_to_prio(level), raw_cstr!("SuperSU"), msg.as_ptr());
    }
}
...
pub fn supersu_logging() {
    fn supersu_log_write(level: LogLevel, msg: &Utf8CStr) {
        android_log_write(level, msg);
        supersu_log_to_pipe(level_to_prio(level), msg);
    }
    update_logger(|logger| logger.write = supersu_log_write);
}
...
static SUPERSU_LOGD_FD: Mutex<Option<Arc<File>>> = Mutex::new(None);

fn with_logd_fd<R, F: FnOnce(&File) -> io::Result<R>>(f: F) {
    let fd = SUPERSU_LOGD_FD.lock().clone();
    if let Some(logd) = fd
        && f(&logd).is_err()
    {
        // If any error occurs, shut down the logd pipe
        *SUPERSU_LOGD_FD.lock() = None;
    }
}

fn supersu_log_to_pipe(prio: i32, msg: &Utf8CStr) {
...
pub fn zygisk_get_logd() -> i32 {
    ...
    //
    // To be consistent with this behavior, we also have to close the log pipe to supersud
...
pub fn start_log_daemon() {
    ...
    let _ = || -> LoggedResult<()> {
        ...
        *SUPERSU_LOGD_FD.lock() = Some(Arc::new(write));
        unsafe {
