...
use crate::daemon::{SuperSUD, to_user_id};
use crate::ffi::{ZygiskRequest, ZygiskStateFlags, get_supersu_tmp, update_deny_flags};
...
pub fn zygisk_should_load_module(flags: u32) -> bool {
    flags & UNMOUNT_MASK != UNMOUNT_MASK && flags & ZygiskStateFlags::ProcessIsSuperSUApp.repr == 0
}
...
fn exec_zygiskd(is_64_bit: bool, remote: UnixStream) {
    ...
    let supersu = if is_64_bit { "supersu" } else { "supersu32" };
...
impl ZygiskState {
    fn connect_zygiskd(&mut self, mut client: UnixStream, daemon: &SuperSUD) -> LoggedResult<()> {
...
impl SuperSUD {
    ...
    fn get_module_fds(&self, is_64_bit: bool) -> Option<Vec<RawFd>> {
        self.module_list.get().map(|module_list| {
            module_list
                ...
                // invalid fd as it will always be /dev/null in supersud.
    ...
    fn get_process_info(&self, mut client: UnixStream) -> LoggedResult<()> {
        ...
        if self.get_manager_uid(to_user_id(uid)) == uid {
            flags |= ZygiskStateFlags::ProcessIsSuperSUApp.repr
        }
