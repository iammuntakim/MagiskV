...
pub(crate) fn hexpatch_init_for_second_stage(writable: bool) {
    ...
    // Redirect original init to supersuinit
    let from = "/system/bin/init";
    let to = "/data/supersuinit";
...
impl SuperSUInit {
    pub(crate) fn hijack_init_with_switch_root(&self) {
        ...
        // supersuinit to /system/bin/init to hijack second stage init.
        //
        ...
        // - chroot to `/system`
        //
        // The trick here is that in SuperSU's first stage init, we can mount supersuinit to /sdcard,
        // and create a symlink at /storage/self/primary pointing to /system/system/bin/init.
        //
        // During init's `SwitchRoot`, it will mount move /sdcard (which is supersuinit)
        ...
        // mounting supersuinit to /system/bin/init, successfully hijacking the second stage init.
        //
        ...
        // First try to mount supersuinit from rootfs to workaround Samsung RKP
        if cstr!("/sdcard")
            ...
            .is_ok()
        {
            ...
            cstr!("/data/supersuinit")
                .bind_mount_to(cstr!("/sdcard"), false)
                .log_ok();
            debug!("Bind mount /data/supersuinit -> /sdcard");
        }
    }
}
